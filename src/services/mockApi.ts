...
import type { AnswerRecord, Student, Task, TaskSetState, QuestionAnswerState, TaskInstance, TaskFigure } from '../types'
import { readJson, writeJson } from '../utils/storage'
import { parseTaskYaml, parseTaskToRuntime } from '../utils/yamlTaskParser'

// ============================================
// FIGURE CONVERSION (YAML -> LaTeX markers)
// ============================================

/**
 * Convert a YAML figure object to LaTeX marker format
 * for backwards compatibility with the latexParser
 */
function figureToLatex(figure: TaskFigure): string {
  if (!figure) return ''
  
  switch (figure.type) {
    case 'triangle': {
      const vertices = figure.vertices
      const lines = Object.entries(vertices)
        .map(([name, v]) => `${name}: ${v.angle}°`)
        .join('\n')
      return `%% FIGURE_TRIANGLE\n${lines}\n%% FIGURE_END\n`
    }
    
    case 'polygon': {
      const lines: string[] = []
      // Add vertices
      for (const [name, pos] of Object.entries(figure.vertices)) {
        const [x, y] = Array.isArray(pos) ? pos : [pos.x, pos.y]
        lines.push(`${name}: (${x}, ${y})`)
      }
      // Add side labels
      if (figure.sides) {
        for (const [side, label] of Object.entries(figure.sides)) {
          lines.push(`side ${side}: ${label}`)
        }
      }
      // Add right angles
      if (figure.right_angles && figure.right_angles.length > 0) {
        lines.push(`right-angle: ${figure.right_angles.join(' ')}`)
      }
      // Add options
      if (figure.show_angles === false) {
        lines.push('show-angles: false')
      }
      if (figure.show_labels === false) {
        lines.push('show-labels: false')
      }
      return `%% FIGURE_POLYGON\n${lines.join('\n')}\n%% FIGURE_END\n`
    }
    ...
    case 'svg': {
      const alt = figure.alt ? ` alt="${figure.alt}"` : ''
      return `%% FIGURE_SVG${alt}\n${figure.content}\n%% FIGURE_END\n`
    }
    
    case 'image': {
      const alt = figure.alt ? ` alt="${figure.alt}"` : ''
      return `%% FIGURE_IMAGE src="${figure.src}"${alt}\n`
    }
    
    // Chart types - not yet supported by latexParser, return empty for now
    case 'bar_chart':
    case 'boxplot':
    case 'rectangles':
    case 'triangles':
    ...
    case 'intersecting_lines':
      // These would need new renderers - skip for now
      return ''
...
function getTasksForUser(userId: string): Task[] {
  ...
  for (const setId of assignedSetIds) {
    ...
    const parts = setTasks.map(instance => {
      ...
      // Convert YAML figure to LaTeX marker format
      const figureLatex = instance.figure ? figureToLatex(instance.figure) : ''
