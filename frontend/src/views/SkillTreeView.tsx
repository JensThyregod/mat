  ...
  return (
    <PageTransition>
      ...
      >
        ...
        <AnimatePresence>
          {selectedSkill && (() => {
            // Build the lineage path (ancestors in order from root to current)
            const lineage: Skill[] = []
            const ancestors = ancestorsMap[selectedSkill.id]
            if (ancestors && ancestors.size > 0) {
              // Get all ancestors and sort by their depth (skills with fewer prerequisites come first)
              const ancestorSkills = Array.from(ancestors)
                .map(id => getSkillById(id))
                .filter(Boolean) as Skill[]
              
              // Sort by y position (top to bottom = earlier to later in tree)
              ancestorSkills.sort((a, b) => a.y - b.y)
              lineage.push(...ancestorSkills)
            }
            lineage.push(selectedSkill)
            
            // Calculate overall lineage progress
                ...
                >
                  ...
                  >
                    ...
                    <div className="skill-modal__title-area">
                      ...
                      <h2 className="skill-modal__name">{selectedSkill.name}</h2>
                      <span className="skill-modal__level">{selectedSkill.level}</span>
                    </div>
                  </motion.div>
                  ...
                  {selectedSkill.status !== 'locked' && (
                    ...
                    >
                      <div className="skill-modal__progress-header">
                        ...
                        <span className="skill-modal__progress-value">{selectedSkill.xp}%</span>
                      </div>
                  ...
                  {/* Lineage Section - Show the skill path */}
                  {lineage.length > 1 && (
                    <motion.div 
                        ...
                        {lineage.map((skill, index) => {
                          ...
                          return (
                            <motion.div
                              ...
                              onClick={() => !isCurrentSkill && setSelectedSkill(skill)}
                            >
